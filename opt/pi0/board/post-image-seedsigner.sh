...
# Only the overlays config.txt can actually use: the two camera sensors
# (kernel-built, see above) and the firmware's overlay-name map. The other
# ~320 rpi-firmware overlays target hardware this image has no drivers for.
